...
public class DoubleArrayTrie {
    ...
    private int fetch(Node parent, List<Node> siblings) {
        ...
        for (int i = parent.left; i < parent.right; i++) {
            ...
            if (cur != prev || siblings.size() == 0) {//siblings.size() == 0可能存在cur为0的情况 词语是有序的
                Node tmp_node = new Node();
                tmp_node.depth = parent.depth + 1;
                tmp_node.code = cur;//如果是一个词的结尾 cur为0
                tmp_node.left = i;
                if (siblings.size() != 0)
    ...
    private int insert(List<Node> siblings) {
        ...
        int pos = ((siblings.get(0).code + 1 > nextCheckPos) ? siblings.get(0).code + 1
                : nextCheckPos) - 1;//找code根nextCheckPos的最大值
        int nonzero_num = 0;
        int first = 0;
        ...
        while (true) {
            ...
            if (used[begin])//begin值只能用一次？
                continue;

            for (int i = 1; i < siblings.size(); i++)
                if (check[begin + siblings.get(i).code] != 0)//check中一直往后找 begin+siblings.get(i).code为0的位置 没找到 begin会+1继续找，知道找到为止
                    continue outer;
        ...
        for (int i = 0; i < siblings.size(); i++) {
            ...
            if (fetch(siblings.get(i), new_siblings) == 0) {
                base[begin + siblings.get(i).code] = (value != null) ? (-value[siblings
                        .get(i).left] - 1) : (-siblings.get(i).left - 1);//如果到啦词的结尾 就赋值为负值
    ...
    public int build(List<String> _key, int _length[], int _value[],
        ...
        fetch(root_node, siblings);//找到当前node下的兄弟节点 节点去重 传入的节点必须是按字典树的顺序传入的，不然会报错
        insert(siblings);//根节点开始插入
    ...
    public List<Integer> commonPrefixSearch(String key, int pos, int len,
        ...
        for (int i = pos; i < len; i++) {
            p = b;//p 在这里是begin第一次root节点中偏移量是1
            n = base[p];//第一次base[begin]=0

            if (b == check[p] && n < 0) {
                result.add(-n - 1);
            }

            p = b + (int) (keyChars[i]) + 1;//p这里开始是p+code
            if (b == check[p])
                b = base[p];
            else
                return result;
    ...
    public static void main(String args[]) {
        ...
        //下面的此必须有序，常用的方式是先构建一个字典树，这里主要讲解双数组压缩树
        words.add("一举");
        words.add("一举一动");
        words.add("一举成名");
        words.add("一举成名天下知");
